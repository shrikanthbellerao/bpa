...
export class D3ZoomableSunburstComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    console.log("hello", this.jsonData)
    /*
        const width = window.innerWidth,
          height = window.innerHeight,
          maxRadius = Math.min(width, height) - 5;
        ...
        const svg = d3.select('svg')
          ...
          // .style('height', `${window.innerWidth/2}px`)
          .attr('viewBox', `${-width / 2} ${-height / 2} ${width} ${height}`)
          .on('click', () => focusOn()); // Reset zoom on canvas click
    
        let root = d3.hierarchy(this.jsonData);
        root.sum(d => d['size']);
    ...
    let chart = function () {
      ...
      const path = g.append("g")
        ...
        .attr("fill", d => {
          ...
          // console.log(`Color for ${d.data.name} is ${color(d.data.name)}`);
      ...
      // Invoked when user clicks the chart
      function clicked(p) {
